...
class PokedexViewmodelTest {
    ...
    fun setUp() {
        ...
        underTest = PokedexViewmodel(schedulerProvider, disposables, pokedexUseCase)
    }

    @Test
    fun `given repository returns ERROR then return Error UiState with proper message`() {
        given(pokedexUseCase.getPokemons()).willReturn(Single.error(Throwable()))

        underTest.pokedexLiveData.observeForever(uiObserver)
        underTest.fetchPokedex()

        with(inOrder(uiObserver)){
            verify(uiObserver).onChanged(UiState.Loading)
            verify(uiObserver).onChanged(UiState.Error(message = "We were unable to get Pokedex data at the moment"))
        }
    }

    @Test
    fun `given repository returns SUCCESS then return Display UiState with data`() {
        given(pokedexUseCase.getPokemons()).willReturn(Single.just(pokemonList))
        ...
        with(inOrder(uiObserver)){
            verify(uiObserver).onChanged(UiState.Loading)
            verify(uiObserver).onChanged(UiState.Display(pokemonList))
        }
    }
}